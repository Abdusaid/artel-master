<template>
    <div class="row">
        ...
        <div class="col s12">
            <div class="">
                <table class="highlight" :class="{ 'content-loading' : importHistoryLoading }">
                    <thead>
                    <tr>
                        <th>Наименование</th>
                        <th>Фирма</th>
                    ...
                    <tbody class="">
                        <tr v-for="(history, index) in importHistory" :class="{'green lighten-3' : (history.status==1), 'yellow lighten-2' : (history.status==2), 'red lighten-2' : (history.status==3)}" @click="chooseImportHistory(index)" :key='index'>
                            ...
                            <td>{{ history.date }}</td>
                            <td>
                                <a class="waves-effect waves-light grey-text right hover-icon" @click.stop="downloadImport(index)"><i class="material-icons">cloud_download</i></a>
                            </td>
                        </tr>
    ...
    export default {
        ...
        methods: {
            ...
            downloadImport: function(index){
                axios.get('/api/notice/word?import_id='+this.importHistory[index].id)
                    .then(response => {
                        window.location = "/api/notice/word?import_id="+this.importHistory[index].id;
                    });
            }
        },
        watch: {
